...
function RegistrationList() {
    ...
    const [activeFilters, setActiveFilters] = useState({gradType: "", pref: "", name: ""});
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await AdminServices.getAllRegStudents(
                    page,
                    limitData,
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            setPage(1);
            setActiveFilters({gradType: gradeType, pref: selectedPref, name: searchName});
        }, 400);
    ...
    const handleShow = () => {
        ...
        setActiveFilters({gradType: gradeType, pref: selectedPref, name: searchName}); // trigger useEffect to fetch with new filters
